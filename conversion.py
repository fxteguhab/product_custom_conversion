...
# METHOD --------------------------------------------------------------------------------------------------------------------
	...
	def get_conversion_qty(self, cr, uid, product_id, uom_id, qty):
		...
		if len(conversion_product_ids) > 0:
			return self.browse(cr, uid, conversion_product_ids[0]).conversion * qty # it must be unique, i.e ids conversion_product_ids just contain 1 id
		else:
			...
			if len(conversion_product_ids) > 0:
				...
				if product_id and (uom.factor_inv == -1 or uom.factor == -1):
